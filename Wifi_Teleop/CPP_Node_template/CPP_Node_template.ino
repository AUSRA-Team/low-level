...
const char * SSID = "zaher55";
const char * WIFI_PASS = "HalaEid55#";
const char * AGENT_IP = "192.168.1.34";
...
void setup() {
  set_microros_transports(); //serial_connection
  // set_microros_wifi_transports((char*)SSID, (char*)WIFI_PASS, (char*)AGENT_IP, AGENT_PORT); // wifi_connection
  ...
  static double data_buffer[3] = {0, 0, 0};
...
void loop() {
  ...
  if (currentTime - lastPingTime >= 1000000) { 
    ...
    if (rmw_uros_ping_agent(50, 1) != RMW_RET_OK) {
      // error_loop(); // Kept commented out until USB is 100% stable
    }
  }
  ...
  if (dt >= (SAMPLE_MS / 1000.0)) {
    ...
    bool braking = (fabs(cmd_t1) < BRAKE_THRESHOLD &&
                fabs(cmd_t2) < BRAKE_THRESHOLD &&
                fabs(cmd_t3) < BRAKE_THRESHOLD);
    float accel_limit = braking ? MAX_BRAKE : MAX_ACCEL;
  ...
  if (max_diff > 0.01f) {
    float max_change = accel_limit * dt;
    float scale = (max_diff > max_change) ? (max_change / max_diff) : 1.0f;
    t1 += diff1 * scale;
    t2 += diff2 * scale;
    t3 += diff3 * scale;
  } else {
